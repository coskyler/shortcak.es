...
router.get("/", async (req: any, res) => {
  try {
    ...
    // collect all slugs
    const slugs = links.map(l => l._id);

    // fetch all analytics in ONE query
    const analyticsDocs = await linkAnalytics
      .find(
        { _id: { $in: slugs } },
        { projection: { totalClicks: 1 } }
      )
      .toArray();

    // build a lookup map: slug -> totalClicks
    const analyticsMap = Object.fromEntries(
      analyticsDocs.map(a => [a._id, a.totalClicks])
    );

    // merge totals into links
    const withClicks = links.map(l => ({
      ...l,
      totalClicks: analyticsMap[l._id] ?? 0
    }));

    return res.json(withClicks);
  } catch (err) {
